...
/// <summary>
/// Phase 51C Editor play-mode verification: vertical seating and 3D flows.
/// Triggered by Captures/phase51c-autotest.flag or menu Shape Nest / Phase 51C Verify Vertical Seating.
/// </summary>
[InitializeOnLoad]
public static class Phase51CPlayModeVerify
{
    private const string FlagPath = "Captures/phase51c-autotest.flag";
    private const string ReportPath = "Captures/phase51c-report.txt";
    private const string SessionKey = "Phase51C.Verify";
    private const string BeforeReference = "Captures/phase51b-t1-all-shapes.png";
    ...
    static Phase51CPlayModeVerify()
    {
        EditorApplication.playModeStateChanged += OnPlayMode;
        EditorApplication.delayCall += TryBeginFromFlag;
    }

    [MenuItem("Shape Nest/Phase 51C Verify Vertical Seating")]
    ...
    private static void TryBeginFromFlag()
    {
        if (File.Exists(FlagPath))
        {
            ...
            SessionState.SetBool("Phase51B.Verify", false);
            SessionState.SetBool(SessionKey, true);
            if (EditorApplication.isPlaying)
            {
                BeginRun();
    ...
    private static void BeginRun()
    {
        ...
        report.Length = 0;
        lastError = null;
        report.AppendLine("Phase 51C Play Mode verification");
        report.AppendLine("anchor bottom-anchored mesh + bounds seating; block surfaceLift=0");
    ...
    private static void Advance()
    {
        switch (step)
        {
            case 0:
                ...
                break;
            case 1:
                Capture("Captures/phase51c-t1-all-shapes.png");
                InspectMeshes("T1 all-shapes");
                InspectVerticalSeating("T1");
                InspectCentering("T1");
                InspectFactoryMeshes();
                break;
            case 2:
                CaptureAngled("Captures/phase51c-t2-angled.png");
                break;
            case 3:
                ...
                break;
            case 4:
                Capture("Captures/phase51c-t3-after-move.png");
                break;
            case 5:
                LoadLevel(Campaign07, "Campaign_07_ChainIntro");
                break;
            case 6:
                Capture("Captures/phase51c-t4-chain.png");
            ...
            case 7:
                ...
                break;
            case 8:
                Capture("Captures/phase51c-t5-nested.png");
            ...
            case 9:
                ...
                break;
            case 10:
                Capture("Captures/phase51c-t9-ice.png");
                InspectIce("T9 ice");
                InspectCentering("T9");
                break;
            case 11:
                ...
                break;
            case 12:
                Capture("Captures/phase51c-t10-shutter.png");
            ...
            case 13:
                ...
                break;
            case 14:
                Capture("Captures/phase51c-t7-magnet-board.png");
                TryMagnetSelect();
                break;
            case 15:
                Capture("Captures/phase51c-t7-magnet-selecting.png");
                CancelMagnet();
                break;
            case 16:
                TryHammerSmash();
                break;
            case 17:
                Capture("Captures/phase51c-t8-hammer.png");
                break;
            default:
    ...
    private static void InspectVerticalSeating(string tag)
    {
        ...
        {
            BoardCamera3D boardCam = Object.FindFirstObjectByType<BoardCamera3D>();
            cam = boardCam != null ? boardCam.Camera : null;
        }
        if (presenter == null)
        {
            report.AppendLine(tag + " vertical no board");
            return;
        }

        float surfaceY = presenter.CellSurfaceWorldY;
        report.AppendLine(tag + " surfaceWorldY=" + surfaceY.ToString("F4"));
        float maxBottomGap = 0f;
        float maxScreenDelta = 0f;
        int counted = 0;
        ...
        for (int i = 0; i < views.Length; i++)
        {
            ...
            float meshMinY = mesh != null ? mesh.bounds.min.y : -1f;
            float meshCenterY = mesh != null ? mesh.bounds.center.y : 0f;
            float bottomGap = renderer != null ? renderer.bounds.min.y - surfaceY : 999f;
            maxBottomGap = Mathf.Max(maxBottomGap, Mathf.Abs(bottomGap - view.SurfaceLift));

            float screenDelta = 0f;
            if (cam != null && renderer != null)
            {
                Transform cells = presenter.transform.Find("Cells");
                if (cells != null)
                {
                    Renderer bestTile = null;
                    float best = 999f;
                    for (int c = 0; c < cells.childCount; c++)
                    {
                        Renderer tileRenderer = cells.GetChild(c).GetComponent<Renderer>();
                        ...
                        float d = Vector3.SqrMagnitude(
                            ...
                            - new Vector3(renderer.bounds.center.x, tileRenderer.bounds.center.y, renderer.bounds.center.z));
                        if (d < best)
                        {
                            best = d;
                            bestTile = tileRenderer;
                        }
                    }

                    if (bestTile != null)
                    {
                        Bounds tb = bestTile.bounds;
                        float tTop = float.MinValue;
                        float tBot = float.MaxValue;
                        float pTop = float.MinValue;
                        float pBot = float.MaxValue;
                        Vector3[] tilePts =
                        {
                            new Vector3(tb.min.x, tb.max.y, tb.min.z), new Vector3(tb.max.x, tb.max.y, tb.max.z),
                            new Vector3(tb.min.x, tb.min.y, tb.min.z), new Vector3(tb.max.x, tb.min.y, tb.max.z)
                        };
                        for (int p = 0; p < tilePts.Length; p++)
                        {
                            float sy = cam.WorldToScreenPoint(tilePts[p]).y;
                            tTop = Mathf.Max(tTop, sy);
                            tBot = Mathf.Min(tBot, sy);
                        }

                        Bounds pb = renderer.bounds;
                        Vector3[] piecePts =
                        {
                            new Vector3(pb.min.x, pb.min.y, pb.min.z), new Vector3(pb.max.x, pb.min.y, pb.max.z),
                            new Vector3(pb.min.x, pb.max.y, pb.min.z), new Vector3(pb.max.x, pb.max.y, pb.max.z)
                        };
                        for (int p = 0; p < piecePts.Length; p++)
                        {
                            float sy = cam.WorldToScreenPoint(piecePts[p]).y;
                            pTop = Mathf.Max(pTop, sy);
                            pBot = Mathf.Min(pBot, sy);
                        }

                        screenDelta = ((pTop + pBot) * 0.5f) - ((tTop + tBot) * 0.5f);
                        maxScreenDelta = Mathf.Max(maxScreenDelta, Mathf.Abs(screenDelta));
                    }
                }
            }

            if (Mathf.Abs(bottomGap - view.SurfaceLift) > 0.004f || meshMinY > 0.02f || meshCenterY < 0.45f || meshCenterY > 0.55f)
            {
                report.AppendLine(
                    "  " + view.ConfiguredShape
                    + (view.ConfiguredAsNest ? " nest" : " block")
                    + " meshMinY=" + meshMinY.ToString("F4")
                    + " meshCenterY=" + meshCenterY.ToString("F4")
                    + " bottomGap=" + bottomGap.ToString("F4")
                    + " lift=" + view.SurfaceLift.ToString("F4")
                    + " screenDelta=" + screenDelta.ToString("F1"));
            }
        }

        report.AppendLine(
            tag + " vertical views=" + counted
            + " maxBottomGapErr=" + maxBottomGap.ToString("F4")
            + " maxAbsScreenCenterDelta=" + maxScreenDelta.ToString("F1"));
    }
